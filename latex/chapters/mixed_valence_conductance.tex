...
The width of a strongly coupled Coulomb peak is controlled by $\Gamma$. Where $\Gamma$ is the sum of the individual tunnelling rates, $\Gamma=\mathrm{\Gamma_L} + \mathrm{\Gamma_R}$. The conductance amplitude depends on the individual tunnelling rates as Eq.~\ref{eq:cond_amp} and can be used to approximate how asymmetrically coupled the quantum dot is. 
% The relation for the overall tunnelling rate and conductance amplitude can be re-arranged to express the ratio of tunnelling between each lead $\mathrm{\Gamma_L}/\mathrm{\Gamma_R}$.
...
% Eq.~\ref{eq:cond_ratio} can be used to show the dot is symmetrically coupled when $\mathrm{\Gamma_L}/\mathrm{\Gamma_R} = 1$, more strongly coupled to the left lead when $\mathrm{\Gamma_L}/\mathrm{\Gamma_R} > 1$ and more strongly coupled to the right lead when $\mathrm{\Gamma_L}/\mathrm{\Gamma_R} < 1$. 
